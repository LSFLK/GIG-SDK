...
func TestThatFixUrlWorks(t *testing.T) {
	...
	baseUrl := "https://www.buildings.gov.lk/index.php?option=com_content&view=category&layout=blog&id=47&Itemid=128&lang=en"
	result := libraries.FixUrl(href, baseUrl)
	expectedResult := "https://www.buildings.gov.lk" + href
...
func TestThatFixUrlWorksForAbsoluteUrl(t *testing.T) {
	href := "https://www.buildings.gov.lk/images/pdf/registered%20suppliers%20and%20service%20%20providers%20for%20the%20year%202019%20-%20g1-office%20stationery.pdf"
	baseUrl := "https://www.buildings.gov.lk/index.php?option=com_content&view=category&layout=blog&id=47&Itemid=128&lang=en"
	result := libraries.FixUrl(href, baseUrl)
	if result != href {
