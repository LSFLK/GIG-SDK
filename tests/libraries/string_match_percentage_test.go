...
func TestThatMatchStringWorksForPartiallyEqualStrings(t *testing.T) {
	matchPercent := libraries.StringMatchPercentage("some what similar string", "som wat similar string")
	if matchPercent != 75 {
		t.Errorf("match string for partially equal strings failed. %d != %d", matchPercent, 75)
	}
}
